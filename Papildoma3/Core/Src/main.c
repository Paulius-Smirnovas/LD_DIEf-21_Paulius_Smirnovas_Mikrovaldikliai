...
float normValue=0;
  ...
  while (1)
  {
    // Start ADC conversiosn
    HAL_ADC_Start(&hadc1);
    // Wait for conversion to be complete
    HAL_ADC_PollForConversion(&hadc1, HAL_MAX_DELAY);
    // Read ADC value
    uint32_t adcValue = HAL_ADC_GetValue(&hadc1);
    normValue=(adcValue-min)/(max-min)*10;
    // Logic to turn on/off LEDs based on adcValue
    // Example threshold values are placeholders and should be adjusted
    if (normValue ==0 && normValue<=0.25 ) { // Adjust threshold as needed
      HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12, GPIO_PIN_SET); // Green LED
    } else {
      HAL_GPIO_WritePin(GPIOD, GPIO_PIN_12, GPIO_PIN_RESET);
    }

    if (normValue > 0.25 && normValue<=0.5) { // Adjust threshold as needed
      HAL_GPIO_WritePin(GPIOD, GPIO_PIN_13, GPIO_PIN_SET); // Orange LED
    } else {
      HAL_GPIO_WritePin(GPIOD, GPIO_PIN_13, GPIO_PIN_RESET);
    }

    if (normValue > 0.5 && normValue<=0.75) { // Adjust threshold as needed
      HAL_GPIO_WritePin(GPIOD, GPIO_PIN_14, GPIO_PIN_SET); // Red LED
    } else {
      HAL_GPIO_WritePin(GPIOD, GPIO_PIN_14, GPIO_PIN_RESET);
    }

    if (normValue > 0.75 && normValue<=1) { // Adjust threshold as needed
